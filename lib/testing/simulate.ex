...
defmodule ValueFlows.Observe.Simulate do
  ...
  def observation(
        base \\ %{},
        has_feature_of_interest \\ nil,
        observed_property \\ nil,
        has_result \\ nil
      ) do
    base
  ...
  def fake_observation!(user, context, overrides) do
    {:ok, observation} =
      Observations.create(
        user,
        context,
        observation_with_req_fields(user, overrides)
      )
  ...
  def observable_property(overrides \\ %{}) do
    Map.put_new_lazy(overrides, :label, &name/0)
  end
  ...
  def fake_observable_phenomenon!(
        user,
        observable_property \\ nil,
        overrides \\ %{}
      )
  ...
  def fake_observable_phenomenon!(user, observable_property, overrides) do
    {:ok, observable_phenomenon} =
      ObservablePhenomenons.create(
        user,
        observable_property,
        observable_phenomenon(overrides)
      )
