...
if Code.ensure_loaded?(Bonfire.API.GraphQL) do
  defmodule ValueFlows.Observe.ObservablePhenomenonsGraphQL do
    ...
    def create_observable_phenomenon(
          %{observable_phenomenon: %{choice_of: choice_of} = params},
          info
        ) do
      with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info),
           {:ok, observable_property} <-
             ValueFlows.Observe.ObservablePropertiesGraphQL.observable_property(
               %{id: choice_of},
               info
             ) do
        ValueFlows.Observe.ObservablePhenomenons.create(
          user,
          observable_property,
          params
        )
    ...
    def update_observable_phenomenon(
          %{observable_phenomenon: %{id: id} = params},
          info
        ) do
    ...
    def delete_observable_phenomenon(%{id: id}, info) do
      with {:ok, user} <- GraphQL.current_user_or_not_logged_in(info),
           {:ok, observable_phenomenon} <-
             Bonfire.Classify.Categories.one(id: id),
           # TODO: check permissions
           {:ok, _} <-
             Bonfire.Classify.Categories.soft_delete(observable_phenomenon) do
        {:ok, true}
      end
    end
    ...
    def observable_phenomenons(page_opts, info) do
      ...
      pages =
        if Bonfire.Common.Extend.module_enabled?(Bonfire.Classify.GraphQL.CategoryResolver) do
          with {:ok, pages} <-
                 Bonfire.Classify.GraphQL.CategoryResolver.categories(
                   Map.put(
                     page_opts,
                     :facet,
                     ValueFlows.Observe.ObservablePhenomenons.facet()
                   ),
                   info
                 ) do
            items =
    ...
    def formula_quantifier_edge(%{"formula_quantifier" => num} = _thing, _, _)
        when not is_nil(num) do
    ...
    def choice_of_edge(%{parent_category: %{id: _} = choice_of} = _thing, _, _) do
      {:ok, ValueFlows.Observe.Classifications.from_classification(choice_of)}
    end

    def choice_of_edge(%{parent_category_id: id} = _thing, _, _)
        when is_binary(id) do
      ValueFlows.Observe.ObservableProperties.one(id: id)
    end

    def choice_of_edge(%{"choice_of" => id} = _thing, _, _)
        when is_binary(id) do
      # deprecated
      ValueFlows.Observe.ObservableProperties.one(id: id)
    end
